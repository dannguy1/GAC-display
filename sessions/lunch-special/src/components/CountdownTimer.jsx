 ...
 * Displays HH:MM:SS format. Shows a configurable ended message when expired.
 */
export default function CountdownTimer({ endHour, endMinute, label, endedLabel }) {
    ...
    if (remaining <= 0) {
        return (
            <div className="countdown countdown--ended">
                <span className="countdown__label">{endedLabel || 'Time is up'}</span>
            </div>
        );
    }
    ...
    return (
        <div className="countdown">
            <span className="countdown__label">{label || 'Ends in'}</span>
